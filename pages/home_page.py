...
class HomePage(BasePage):
    ...
    def check_order_title(self):
        return self.find_locator(LocatorsHome.header_page_title).is_displayed()

    @allure.step('Принять куки')
    def accept_cookie_home_page(self):
        self.click_button(LocatorsHome.accept_cookies_button)

    @allure.step('Скролл и клик на кнопку Заказать')
    def scroll_and_click_on_the_order_button(self):
        self.scroll_to_locator(LocatorsHome.order_button)
        self.click_button(LocatorsHome.order_button)

    @allure.step('Переход к списку вопросов')
    def scroll_to_questions(self):
        self.scroll_to_locator(LocatorsHome.questions_title)
